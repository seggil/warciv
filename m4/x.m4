dnl WC_CHECK_X_LIB(LIBRARY, FUNCTION [, ACTION-IF-FOUND [,
...
dnl   WC_CHECK_X_LIB(X11, XOpenDisplay, , AC_MSG_ERROR("Need X11"))
dnl   WC_CHECK_X_LIB(Xext, XShapeCombineMask)
dnl     [etc.]
dnl   LIBS="$X_LIBS $X_EXTRA_LIBS $LIBS"
dnl
AC_DEFUN([WC_CHECK_X_LIB], [
 ...
 if eval "test \"`echo '$ac_cv_lib_'$ac_lib_var`\" = yes"; then
  ...
  ifelse([$3], ,
   [changequote(, )dnl
    ...
    # The HAVE_LIBX** values are defined in WC_CHECK_X_LIB, but we need an
...
dnl WC_EXPAND_DIR(VARNAME, DIR)
dnl expands occurrences of ${prefix} and ${exec_prefix} in the given DIR,
dnl and assigns the resulting string to VARNAME
dnl example: WC_EXPAND_DIR(LOCALEDIR, "$datadir/locale")
...
wc_xpm_default=/usr/local
if test "$xpm_libdir" = "yes"; then
    xpm_libdir="no"
    wc_xpm_default_lib="$wc_xpm_default/lib"
    for wc_extension in a so sl; do
        if test -r $wc_xpm_default_lib/libXpm.$wc_extension; then
            xpm_libdir=$wc_xpm_default_lib
            break
        fi
    done
fi
...
if test "$xpm_incdir" = "yes"; then
    xpm_incdir="no"
    wc_xpm_default_inc="$wc_xpm_default/include"
    if test -r $wc_xpm_default_inc/X11/xpm.h; then
        xpm_incdir=$wc_xpm_default_inc
    elif test -r $wc_xpm_default_inc/xpm.h; then
        xpm_incdir=$wc_xpm_default_inc
    xpm_h_no_x11=yes
    fi
fi
...
dnl WC_CHECK_X_PROTO_DEFINE(DEFINED-VARIABLE)
dnl
...
dnl
dnl The following variables are output:
dnl   wc_x_proto_value      -- contains the value to which
dnl             the DEFINED-VARIABLE is set,
dnl             or "" if it has no known value.
dnl
dnl Example use:
dnl   WC_CHECK_X_PROTO_DEFINE(FUNCPROTO)
dnl   if test -n "$wc_x_proto_value"; then
dnl     AC_DEFINE_UNQUOTED(FUNCPROTO, $wc_x_proto_value)
dnl   fi
dnl
AC_DEFUN([WC_CHECK_X_PROTO_DEFINE],
[AC_REQUIRE([WC_CHECK_X_PROTO_FETCH])dnl
AC_MSG_CHECKING(for Xfuncproto control definition $1)
# Search for the requested defined variable; return it's value:
wc_x_proto_value=
for wc_x_define in $wc_x_proto_defines; do
  wc_x_val=1
  eval `echo $wc_x_define | sed -e 's/=/ ; wc_x_val=/' | sed -e 's/^/wc_x_var=/'`
  if test "x$wc_x_var" = "x$1"; then
    wc_x_proto_value=$wc_x_val
    break
  fi
done
if test -n "$wc_x_proto_value"; then
  AC_MSG_RESULT([yes: $wc_x_proto_value])
else
  AC_MSG_RESULT([no])
fi
])

dnl WC_CHECK_X_PROTO_FETCH
dnl
dnl This macro fetches the Xfuncproto control definitions.
dnl (Intended to be called once from WC_CHECK_X_PROTO_DEFINE.)
dnl
dnl The following variables are output:
dnl   wc_x_proto_defines    -- contains the list of defines of
dnl             Xfuncproto control definitions
dnl             (defines may or may not include
dnl             the -D prefix, or an =VAL part).
dnl
dnl Example use:
dnl   AC_REQUIRE([WC_CHECK_X_PROTO_FETCH])
dnl
AC_DEFUN([WC_CHECK_X_PROTO_FETCH],
...
EOF
    if (xmkmf) >/dev/null 2>/dev/null && test -f Makefile; then
      ...
      for wc_x_value in 15 11 3 1 ""; do
    WC_CHECK_X_PROTO_FUNCPROTO_COMPILE($wc_x_value)
    if test "x$wc_x_proto_FUNCPROTO" != "xno"; then
      break
    fi
      done
      if test "x$wc_x_proto_FUNCPROTO" != "xno"; then
    wc_x_proto_defines="$wc_x_proto_defines FUNCPROTO=$wc_x_proto_FUNCPROTO"
    AC_MSG_RESULT([yes, determined: $wc_x_proto_FUNCPROTO])
      else
    AC_MSG_RESULT([no, cannot determine])
      ...
      dnl Try in order of preference...
      for wc_x_value in 1 ""; do
    WC_CHECK_X_PROTO_NARROWPROTO_WORKS($wc_x_value)
    if test "x$wc_x_proto_NARROWPROTO" != "xno"; then
      break
    fi
      done
      if test "x$wc_x_proto_NARROWPROTO" != "xno"; then
    wc_x_proto_defines="$wc_x_proto_defines NARROWPROTO=$wc_x_proto_NARROWPROTO"
    AC_MSG_RESULT([yes, determined: $wc_x_proto_NARROWPROTO])
      else
    AC_MSG_RESULT([no, cannot determine])
      fi
      AC_MSG_CHECKING(whether Xfuncproto was determined)
      if test -n "$wc_x_proto_defines"; then
    AC_MSG_RESULT([yes: $wc_x_proto_defines])
      else
    AC_MSG_RESULT([no])
      fi
    fi
  ...
  wc_x_proto_defines=$with_x_funcproto
  AC_MSG_RESULT([yes, given: $wc_x_proto_defines])
fi
])

dnl WC_CHECK_X_PROTO_FUNCPROTO_COMPILE(FUNCPROTO-VALUE)
dnl
...
dnl
dnl The following variables are output:
dnl   wc_x_proto_FUNCPROTO  -- contains the passed-in
dnl             FUNCPROTO-VALUE if Xfuncproto.h
dnl             compiled, or "no" if it did not.
dnl
dnl Example use:
dnl   WC_CHECK_X_PROTO_FUNCPROTO_COMPILE($wc_x_value)
dnl   if test "x$wc_x_proto_FUNCPROTO" != "xno"; then
dnl     echo Compile using FUNCPROTO=$wc_x_proto_FUNCPROTO
dnl   fi
dnl
AC_DEFUN([WC_CHECK_X_PROTO_FUNCPROTO_COMPILE],
[AC_REQUIRE([AC_PATH_XTRA])dnl
AC_LANG_SAVE
AC_LANG_C
wc_x_proto_FUNCPROTO=no
if test "x$1" = "x"; then
  wc_x_compile="#undef FUNCPROTO"
else
  wc_x_compile="#define FUNCPROTO $1"
fi
wc_x_save_CFLAGS="$CFLAGS"
...
exit (0)
  ...
  [wc_x_proto_FUNCPROTO=$1])
CFLAGS="$wc_x_save_CFLAGS"
AC_LANG_RESTORE
])

dnl WC_CHECK_X_PROTO_NARROWPROTO_WORKS(NARROWPROTO-VALUE)
dnl
...
dnl   wc_x_proto_FUNCPROTO  -- the value to use for FUNCPROTO.
dnl
dnl The following variables are output:
dnl   wc_x_proto_NARROWPROTO    -- contains the passed-in
dnl             NARROWPROTO-VALUE if the test
dnl             worked, or "no" if it did not.
dnl
dnl Example use:
dnl   WC_CHECK_X_PROTO_NARROWPROTO_WORKS($wc_x_value)
dnl   if test "x$wc_x_proto_NARROWPROTO" != "xno"; then
dnl     echo Compile using NARROWPROTO=$wc_x_proto_NARROWPROTO
dnl   fi
dnl
AC_DEFUN([WC_CHECK_X_PROTO_NARROWPROTO_WORKS],
[AC_REQUIRE([AC_PATH_XTRA])dnl
AC_LANG_SAVE
AC_LANG_C
wc_x_proto_NARROWPROTO=no
if test "x$1" = "x"; then
  wc_x_works="#undef NARROWPROTO"
else
  wc_x_works="#define NARROWPROTO $1"
fi
if test "x$wc_x_proto_FUNCPROTO" = "x"; then
  wc_x_compile="#define FUNCPROTO 1"
else
  wc_x_compile="#define FUNCPROTO $wc_x_proto_FUNCPROTO"
fi
wc_x_save_CFLAGS="$CFLAGS"
