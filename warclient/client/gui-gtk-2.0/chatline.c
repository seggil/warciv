...
static struct tag_pattern_list tagpats;
static bool tagpats_initialized = FALSE;
...
{
  if (!tagpats_initialized) {
    tag_pattern_list_init (&tagpats);
    tagpats_initialized = TRUE;
  }
}
/**************************************************************************
...
                              ...
                              gpointer user_data)
{
  ...
  if (tmptagpats) {
    tag_pattern_list_free (tmptagpats);
  }
...
/**************************************************************************
  ...
  with tag_pattern_list_free when no longer needed.
**************************************************************************/
struct tag_pattern_list *tag_pattern_list_clone (struct tag_pattern_list *old)
{
  struct tag_pattern_list *new;
...
static void clear_callback (GtkWidget *w, gpointer user_data)
{
  ...
  tag_pattern_list_iterate (*tmptagpats, ptagpat) {
    g_hash_table_replace (tags_to_delete,
                          mystrdup (ptagpat->tag_name), NULL);
  } tag_pattern_list_iterate_end;
