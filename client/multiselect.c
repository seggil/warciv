...
void multi_select_cat(int dest,int src)
{
	...
	multi_select_update(src);
	if(!multi_select_size(src))
		return;
	multi_select_update(dest);
...
{
	msassert(multi);
	if(multi==0&&multi_select_size(0)==1)
		return;

	struct unit *punit_next_focus=NULL;
		
	unit_list_iterate(multi_selection[multi].ulist,punit)
	{
		if(punit!=player_find_unit_by_id(game.player_ptr,punit->id))
		{
			unit_list_unlink(&multi_selection[multi].ulist,punit);
			if(punit==multi_selection[multi].punit_focus)
				multi_selection[multi].punit_focus=NULL;
		}
		else
			punit_next_focus=punit;
	} unit_list_iterate_end;
	if(!multi_select_size(multi))
		multi_selection[multi].punit_focus=NULL;
	else if(!multi_selection[multi].punit_focus&&punit_next_focus)
	{
		multi_selection[multi].punit_focus=punit_next_focus;
		if(multi==0)
			set_unit_focus(punit_next_focus);
	}
	if(multi==0)
	{
		update_unit_info_label(get_unit_in_focus());
		multi_select_blink_update();
	}
}
...
void add_unit_to_delayed_goto(struct tile *ptile)
{
	...
	unit_list_iterate(*ulist,punit)
	{
		if((delayed_goto_place==PLACE_ON_CONTINENT&&punit->tile->continent!=punit_focus->tile->continent)
