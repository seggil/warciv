...
# if REPLAY
  printf("DIPLOMACY_ACCEPT_TREATY\n"); /* done */
...
# if REPLAY
  printf("DIPLOMACY_INIT_MEETING counterpart=%d initiated_from=%d\n",
...
# if REPLAY
  printf("DIPLOMACY_CANCEL_MEETING counterpart=%d initiated_from=%d\n",
          counterpart, initiated_from); /* done */
# endif
  pdialog = find_diplomacy_dialog(counterpart);
  if (!pdialog) {
...
# if REPLAY
  printf("DIPLOMACY_CREATE_CLAUSE"
...
# if REPLAY
  printf("DIPLOMACY_REMOVE_CLAUSE"
         "counterpart=%d giver=%d type=%d value=%d\n",
         counterpart, giver, type, value); /* done */
# endif
  pdialog = find_diplomacy_dialog(counterpart);
...
static void popup_add_menu(GtkMenuShell *parent, gpointer data)
{
  ...
  /* Advances. */
  {
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    g_signal_connect(item, "activate",
                     G_CALLBACK(diplomacy_dialog_all_tech_callback),
                     GINT_TO_POINTER((plr0->player_no << 8)
                                     | (plr1->player_no)));
