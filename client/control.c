...
static struct unit *find_best_focus_candidate(bool accept_current)
{
  ...
  unit_list_iterate(game.player_ptr->units, punit) {
	...
	&& punit->moves_left > 0
	&& !punit->done_moving && !punit->ai.control) {
...
void schedule_delayed_airlift(struct tile *ptile)
{
  ...
  my_snprintf(txt, sizeof(txt),
	      _("Warclient: Scheduling delayed airlift for (%d, %d)."),
	      ptile->x, ptile->y);
  append_output_window(txt);
...
void request_unit_execute_delayed_goto(void)
{
  ...
  append_output_window(_("Warclient: Executing delayed goto."));
  connection_do_buffer(&aconnection);
  delayed_goto_data_list_iterate(DGqueue, dgd) {
    if (unit_limit && (++counter > unit_limit))
      break;			/* unit_limit = 0 means unlimited */
...
void request_unit_clear_delayed_orders(void)
{
  ...
  append_output_window(_("Warclient: Delayed orders queue cleared."));
}
...
static int rally_point_unref(struct rally_point *rp)
{
  ...
  if (rp->refcount == 0) {
    freelog(LOG_DEBUG, "refcount for rally_point %p reached 0, freeing", rp);
...
bool can_unit_do_connect(struct unit * punit, enum unit_activity activity)
{
  ...
  switch (activity) {
  case ACTIVITY_ROAD:
    return terrain_control.may_road && unit_flag(punit, F_SETTLERS)
  ...
  case ACTIVITY_IRRIGATE:
    ...
    return (terrain_control.may_irrigate && unit_flag(punit, F_SETTLERS)
...
{
  dsend_packet_unit_diplomat_action(&aconnection, dipl_id, action, target_id,
				    value);
}
...
void request_unit_mass_load(struct unit *punit)
{
  ...
  if (goto_mode && ctrl_state) {	/* goto for all units of the selected unit type on the same tile */
    if (goto_mode == 3)
      ulist = &game.player_ptr->units;
    else
      ulist = &punit_focus->tile->units;
    unit_list_iterate(*ulist, tunit) {
      if ((goto_mode == 2 || tunit->type == punit_focus->type)
	  && (tunit->tile->continent == punit_focus->tile->continent)) {
	if (unit_satisfies_filter(tunit))
	  request_unit_load(tunit, NULL);
      }
    } unit_list_iterate_end;
...
void request_unit_unload(struct unit *pcargo)
{
  ...
  if (can_client_issue_orders()
      && ptrans && can_unit_unload(pcargo, ptrans)
...
void request_unit_pillage(struct unit *punit)
{
  ...
  enum tile_special_type psworking = get_unit_tile_pillage_set(punit->tile);
...
void do_move_unit(struct unit *punit, struct unit *target_unit)
{
  ...
  if (!was_teleported
      && punit->activity != ACTIVITY_SENTRY && punit->transported_by == -1) {
  ...
  if (game.player_idx == punit->owner
      && auto_center_on_unit && !unit_has_orders(punit)
...
void do_map_click(struct tile *ptile, enum quickselect_type qtype)
{
  ...
  } else if (unit_list_size(&ptile->units) == 0 && !pcity && punit_focus) {
    maybe_goto = keyboardless_goto;
...
void key_unit_build_wonder(void)
{
  ...
  if (goto_mode && ctrl_state) {
    ...
    if (unit_flag(punit_focus, F_HELP_WONDER))
      request_unit_caravan_action(punit_focus,
...
void key_unit_diplomat_actions(void)
{
  ...
  if (punit_focus && is_diplomat_unit(punit_focus)
