...
static struct pepsetting static_pepsettings[]=
{
	...
	PSGEN_BOOL(PAGE_MS,multi_select_map_selection,N_("Multi-selection at map"),MS_MAP),
	...
	PSGEN_INT(PAGE_TRADE,my_ai_trade_plan_time_max,N_("Maximum time (in seconds) allowed to trade planning calcul"),N_("0: No limit"),
		MY_AI_TRADE_TIME),
	PSGEN_BOOL(PAGE_TRADE,my_ai_trade_plan_recalculate_auto,N_("Automaic trade planning calcul"),MY_AI_TRADE_PLAN_AUTO),
...
#define load(type,way,data)	\
	...
	{	\
		freelog(LOG_ERROR,"Cannot load dynamics settings (%s: line %d, bad " #type " '%s'), aborting...",__FILE__,__LINE__,way);	\
		goto free_datas;	\
	}	\
	if(!(data))	\
	{	\
		freelog(LOG_NORMAL,"Cannot load '%s' (%s: line %d, " #type " not found)",way,__FILE__,__LINE__);	\
		error_count++;	\
		if(errors_max&&error_count>=errors_max)	\
		{	\
			freelog(LOG_ERROR,"Cannot load dynamics settings, too much warning (%d/%d), aborting...",error_count,errors_max);	\
			goto free_datas;	\
		}	\
	}	\
	else
...
#define load_owner(type,way,data)	\
	...
	{	\
		freelog(LOG_NORMAL,"Cannot load '%s' (%s: line %d, " #type " has a bad owner)",way,__FILE__,__LINE__);	\
		data=NULL;	\
	}	\
	else
