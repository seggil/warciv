                                 ...
                                 char *buffer, size_t buffer_len)
{
  ...
  if (draw_city_production_buy_cost) {
    my_snprintf (coststr, sizeof(coststr), " %d", city_buy_cost (pcity));
  } else {
    coststr[0] = '\0';
  }
  ...
  if (pcity->is_building_unit) {
    ...
    if (turns < 999) {
      ...
      my_snprintf(buffer, buffer_len, "%s -%s",
                  punit_type->name, coststr);
    }
  } else {
    ...
    if (get_current_construction_bonus(pcity, EFT_PROD_TO_GOLD) > 0) {
      ...
      my_snprintf(buffer, buffer_len, "%s %d%s",
		  pimprovement_type->name, turns, coststr);
    } else {
      my_snprintf(buffer, buffer_len, "%s -%s",
                  pimprovement_type->name, coststr);
    }
  }
}
				      ...
				      enum color_std *traderoutes_color)
{
  ...
  if (!draw_city_traderoutes) {
    ...
    *traderoutes_color = COLOR_STD_WHITE;
    return;
  }
...
static void draw_traderoutes_for_city (struct city *src_pcity)
{
  ...
  //*pepeto*
 draw_trade_routes(my_ai_trade_plan_get(),COLOR_STD_RED)
  draw_trade_routes(&src_pcity->trade_routes,COLOR_STD_YELLOW)
  draw_trade_routes(estimate_non_ai_trade_route(),COLOR_STD_ORANGE)
}
