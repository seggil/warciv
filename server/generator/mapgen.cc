...
/**************************************************************************
  ...
  the map.server.steepness value, so increasing map.mountains will result in
...
static void make_relief(void)
{
  ...
  hmap_mountain_level = ((hmap_max_level - hmap_shore_level)
                         * (100 - map.server.steepness)) / 100 + hmap_shore_level;
...
static void make_rivers(void)
{
  ...
  while (current_riverlength < desirable_riverlength
    ...
    if (
      ...
      for (i = 0; i < map.info.xsize * map.info.ysize; i++) {
        river_map[i] = 0;
      }
...
/**************************************************************************
  ...
  1) with map.server.landpercent it generates a ocean/grassland map
...
{
  map_free();
  map.info.topology_id = old_topology_id;
...
FIXME: Some continent numbers are unused at the end of this function, fx
  ...
  not map.info.xsize and map.info.ysize will be used.
**************************************************************************/
void map_fractal_generate(bool autosize)
{
  ...
  if (map.server.generator != 0) {
    int old_topology_id = map.info.topology_id;

    if (map.server.generator == 4 || map.server.generator == 5) {
      /* Other topologies would break the gen. */
      map.info.topology_id = TF_WRAPX;
    } else if (map.server.generator == 6 || map.server.generator == 7) {
      /* Don't support topologies > 3 */
      map.info.topology_id &= TF_WRAPX | TF_WRAPY;
    }
    ...
    if (map.server.generator == 1) {
      make_random_hmap(MAX(1, 1 + SQSIZE
                           - (map.server.startpos ? game.info.nplayers / 4 : 0)));
    }
  ...
  if (!map.server.have_specials) {
    add_specials(map.server.riches);
  }
                                                   ...
                                                   *const pstate)
{
  ...
  assert((pstate->e - pstate->w) < map.info.xsize);
  assert((pstate->s - pstate->n) > 0);
  assert((pstate->s - pstate->n) < map.info.ysize);
                        ...
                        const struct gen234_state *const pstate)
{
  ...
  *bucket -= i * capac;
  ...
  while (i > 0 && (failsafe--) > 0) {
    ...
    if (map_get_continent(ptile) == (int)pstate->isleindex
        && not_placed(ptile)) {
      ...
      if ( ( i*3>k*2
           ...
           &&( ! is_cardinally_adj_to_ocean(ptile)
               || (int)myrand(100) < coast ))
      {
        if (map_colatitude(ptile) < COLD_LEVEL) {
          if ((int)myrand(cold0_weight + cold1_weight) < cold0_weight) {
            map_set_terrain(ptile, cold0);
            map_set_placed(ptile);
          } else {
            map_set_terrain(ptile, cold1);
            map_set_placed(ptile);
          }
        } else {
          if ((int)myrand(warm0_weight + warm1_weight) < warm0_weight) {
            map_set_terrain(ptile, warm0);
            map_set_placed(ptile);
          } else {
            map_set_terrain(ptile, warm1);
            map_set_placed(ptile);
          }
        }
      }
  ...
  while (i > 0 && (failsafe--) > 0) {
    ...
    if (map_get_continent(ptile) == (int)pstate->isleindex
      ...
      if ((i * 3 > k * 2
           ...
           || myrand(100) < 50)
          && (!is_cardinally_adj_to_ocean(ptile) || (int)myrand(100) < coast)) {
...
static unsigned long checkmass;
...
static bool create_island(int islemass, struct gen234_state *pstate)
{
  ...
  while (i > 0 && tries-->0) {
    ...
    if ((!near_singularity(ptile) || myrand(50) < 25 )
      ...
      hmap(ptile) = 1;
      i--;
      if (ptile->nat_y >= pstate->s - 1 && pstate->s < map.info.ysize - 2) {
        pstate->s++;
      }
      if (ptile->nat_x >= pstate->e - 1 && pstate->e < map.info.xsize - 2) {
        pstate->e++;
      }
                        ...
                        int min_specific_island_size)
{
  ...
  if (islemass == 0) {
    ...
    if ((unsigned)islemass > (map.info.ysize - 6) * (map.info.ysize - 6)) {
      islemass = (map.info.ysize - 6) * (map.info.ysize - 6);
    }

    if ((unsigned)islemass > (map.info.xsize - 2) * (map.info.xsize - 2)) {
      islemass = (map.info.xsize - 2) * (map.info.xsize - 2);
    }
...
static void mapgenerator2(void)
{
  ...
  pstate->totalmass = ((map.info.ysize - 6 - spares) * map.server.landpercent
                       * (map.info.xsize - spares)) / 100;
  ...
  if (checkmass > map.info.xsize + map.info.ysize + totalweight) {
...
static void mapgenerator3(void)
{
  ...
  unsigned int islandmass;
  unsigned int landmass;
  long unsigned int size;
  unsigned int maxmassdiv6 = 20;
  unsigned int bigislands;
  struct gen234_state state;
  struct gen234_state *pstate = &state;
  ...
  pstate->totalmass =
      ((map.info.ysize - 6 - spares) * map.server.landpercent * (map.info.xsize - spares)) /
      100;

  bigislands= game.info.nplayers;

  landmass = (map.info.xsize * (map.info.ysize - 6) * map.server.landpercent)/100;
  /* subtracting the arctics */
  if (landmass > 3 * map.info.ysize + game.info.nplayers * 3){
    landmass -= 3 * map.info.ysize;
  }
  ...
  if (map.info.xsize < 40 || map.info.ysize < 40 || map.server.landpercent > 80) {
  ...
  while (pstate->isleindex - 2 <= bigislands
         && checkmass > islandmass
         && ++j < 500) {
  ...
  } else if (checkmass > map.info.xsize + map.info.ysize) {
    freelog(LOG_VERBOSE, "%ld mass left unplaced", checkmass);
  }
}
...
static void mapgenerator4(void)
{
  ...
  if (game.info.nplayers < 2 || map.server.landpercent > 80) {
    notify_conn(NULL, _("Server: Cannot create a such map with generator %d"),
                map.server.generator);
                             ...
                             int neck_offset)
{
  ...
  /* make perm ocean */
  {
    ...
    for (cy = map.info.ysize / 2; cy != top_location; cy += direction) {
  ...
  for (cx = x; cx < x + width; cx++) {
    for (cy = y; cy != y + (head_height + 1) * direction; cy += direction) {
      ...
      if(ptile != NULL) {
        ...
        hmap(ptile) = myrand((hmap_max_level * map.server.steepness)/100) + hmap_shore_level;
      }
    }
  }
  ...
  map.server.start_positions[player_number].tile = native_pos_to_tile(x + width / 2, y);
  map.server.start_positions[player_number].nation = NO_NATION_SELECTED;
...
static bool mapgenerator67(bool make_roads)
{
  unsigned int peninsulas = game.info.nplayers;
  unsigned int peninsulas_on_one_side = (peninsulas + 1) / 2;
  int isthmus_width = 10;
  int neck_height = (map.info.ysize / 8) | 1;
  unsigned int polar_height = 3;
  unsigned int max_peninsula_width_with_ocean = (map.info.xsize - isthmus_width) / (peninsulas_on_one_side);
  unsigned int max_peninsula_width = max_peninsula_width_with_ocean / 1.5f;
  unsigned int peninsula_separation = max_peninsula_width_with_ocean - max_peninsula_width;

  unsigned int max_peninsula_height = map.info.ysize / 2 - polar_height - neck_height;
  unsigned int neck_width = MIN(6,max_peninsula_width - 1);
  unsigned int min_peninsula_width = neck_width;
  unsigned int min_peninsula_height = MIN(neck_height, 5);
  unsigned int peninsula_area = MAX((max_peninsula_width * max_peninsula_height *
                                     map.server.landpercent) / 100,
  ...
  y = map.info.ysize / 2;
  for (x = isthmus_width; x < map.info.xsize; x++) {
  ...
  for (y = polar_height; y < map.info.ysize - polar_height; y++) {
    ...
    hmap(native_pos_to_tile(x, y)) = myrand((hmap_max_level * map.server.steepness)/100) + hmap_shore_level;
  }
  ...
  for (i = 0; i < game.info.nplayers; i++) {
    ...
    unsigned int direction = (i < peninsulas_on_one_side) ? ~0 : 1;
    unsigned int index = (direction == ~0U) ? i : i - peninsulas_on_one_side;
    unsigned int width = min_peninsula_width
                         + myrand(max_peninsula_width - min_peninsula_width + 1);
    unsigned int height = CLIP(min_peninsula_height,
                               peninsula_area / width + neck_height,
                               max_peninsula_height);
    unsigned int neck_offset = myrand(width - neck_width + 1);;

    unsigned int x_offset = myrand(max_peninsula_width - width + 1);
    if (index == 0) {
    ...
    if (direction == ~0U)
      y = height + polar_height;
    else
      y = map.info.ysize - 1 - height - polar_height;
  ...
  {
    int consider_height = map.info.ysize - 2 * polar_height;
    int consider_height_for_isles = map.info.ysize
                                    - 2 * (min_peninsula_height + polar_height);
    int desired_squares =
      (consider_height * map.info.xsize * map.server.landpercent) / 200;
    int bailout_number = desired_squares * 30;/* to prevent very long loops*/
    int seed = (consider_height * map.info.xsize) / 150; /* number of seed islands */
    ...
    freelog(LOG_VERBOSE, "Seed is %i, shorelevel is %i, seedcoeff %i",
            seed, hmap_shore_level, (consider_height * map.info.xsize) / 150);
    while (bailout_number > 0 && seed > 0) {
      x = myrand(map.info.xsize);
      ...
      if (map_get_terrain(native_pos_to_tile(x, y)) == T6_TEMP_OCEAN) {
        ...
        hmap(native_pos_to_tile(x, y)) =
            myrand((hmap_max_level * map.server.steepness) / 100) + hmap_shore_level;
        seed--;
  ...
  if (make_roads) {
    unsigned int last_x;
    unsigned int middle_x = isthmus_width / 2;
    last_x = middle_x;
    for (y = polar_height - 1; y < map.info.ysize - polar_height + 1; y++) {
      unsigned int best_x = middle_x;
...
void startpos_new(int x, int y, Nation_Type_id nation)
{
  assert(map.server.num_start_positions < game.info.nplayers);
  map.server.start_positions[map.server.num_start_positions].tile = native_pos_to_tile(x, y);
  map.server.start_positions[map.server.num_start_positions].nation = nation;
...
void terrain_chance_init(void)
{
  double temp_rate = (double)map.server.temperature * map.server.temperature / 500;
  double steep_rate = (double)map.server.steepness * map.server.steepness / 180;
...
void copy_map(struct gen8_map *dest, int x, int y, struct gen8_map *src,
              ...
              player_t **pplayers, size_t num)
{
  unsigned int i, xc, yc;
  unsigned xsize = xmax - xmin;
  unsigned ysize = ymax - ymin;
  unsigned idx = 0;

  for (xc = 0; xc < xsize; xc++) {
    for (yc = 0; yc < ysize; yc++) {
      unsigned int sx = xmin + xc;
      unsigned int sy = ymin + yc;
      unsigned int dx = x + xc;
      unsigned int dy = y + yc;

      normalize_coord(src, (int*)&sx, (int*)&sy);
      normalize_coord(dest, (int*)&dx, (int*)&dy);
      ...
      if (new_startpos) {
        for (i = 0; i < (unsigned)src->tiles[sx][sy].start_pos; i++) {
          if (pplayers) {
            assert(idx < num);
...
struct gen8_map *create_fair_island(int size, int startpos)
{
  ...
  for (tx = xmin; tx <= xmax; tx++) {
    for (ty = ymin; ty <= ymax; ty++) {
      ...
      if ((int)myrand(1000) < map.server.riches) {
  ...
  /* Make river */
  n = (size * myrand(map.server.wetness * map.server.wetness)) / 50000;
  c = i = 0;
  while (c < n) {
    i++;
    if (i > 10000) {
                                         ...
                                         player_t **pplayers, size_t num)
{
  unsigned int x, y;
  unsigned int rx, ry;
  unsigned int rsx, rsy;
  unsigned int xmax = pmap->xsize;
  unsigned int ymax = pmap->ysize;
  const unsigned int RANDOM_LENGHT = 5;
...
#define cases_iterate \
  ...
  switch(game.server.teamplacementtype) {
    case 2:
      for (y = 0; y < (unsigned)pmap->ysize; y++) {
        for (x = 0; x < (unsigned)pmap->xsize; x++) {
          cases_iterate;
        }
      }
      return FALSE;
...
static bool mapgenerator89(bool team_placement)
{
  ...
  for (iter = 0; iter < 500 && !done; iter++) {
    ...
    /* Initialize */
    startpos_init();
    pmap = create_map(map.info.xsize, map.info.ysize);
    ...
    for (x = 0; x < map.info.xsize; x++) {
      for (y = 0; y < map.info.ysize; y++) {
        ...
        if (map_get_terrain(ptile) != OLD_TERRAIN_OCEAN) {
          ...
          adjacent_tiles_pos_iterate(pmap, (int)x, (int)y, tx, ty) {
    ...
    if (team_placement && game.ext_info.teamplacement) {
      ...
      int px[MAX_NUM_TEAMS], py[MAX_NUM_TEAMS], dx = 0, dy = 0;
      ...
      if (topo_has_flag(TF_WRAPX)) {
        dx = myrand(map.info.xsize);
      }
      if (topo_has_flag(TF_WRAPY)) {
        dy = myrand(map.info.ysize);
      }
      for (i = 0; i < teams; i++) {
        px[i] = (map.info.xsize * (2 * i + 1)) / (2 * teams) + dx;
        py[i] = (map.info.ysize * (2 * i + 1)) / (2 * teams) + dy;
