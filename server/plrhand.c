		    ...
		    bool saving_bulbs, int next_tech)
{
   ...
   */
  if (tech_found == game.ruleset_control.rtech_cathedral_plus
      || tech_found == game.ruleset_control.rtech_cathedral_minus
      || tech_found == game.ruleset_control.rtech_colosseum_plus
      || tech_found == game.ruleset_control.rtech_temple_plus) {
...
static void tech_researched(struct player* plr)
{
  ...
  if(!is_future_tech(tech) && game.ruleset_game.tech_leakage) {
...
void init_tech(struct player *plr)
{
  ...
  plr->research.changed_from = A_NONE;
...
void give_initial_techs(struct player* plr)
{
  ...
  for (i = 0; i < MAX_NUM_TECH_LIST; i++) {
    if (game.ruleset_game.global_init_techs[i] == A_LAST) {
      break;
    }
    found_new_tech(plr, game.ruleset_game.global_init_techs[i], FALSE, TRUE, A_NONE);
  }
...
static void start_revolution(struct player *pplayer)
{
  ...
  if (pplayer->revolution_finishes > 0) {
    ...
    pplayer->revolution_finishes = game.info.turn + game.server.revolution_length;
  }
  ...
  if (pplayer->revolution_finishes <= game.info.turn
      && pplayer->target_government != game.ruleset_control.government_when_anarchy) {
...
static Nation_Type_id pick_available_nation(Nation_Type_id *choices)
{
  int *nations_used, i, num_nations_avail = game.ruleset_control.playable_nation_count, pick;
...
static struct player *split_player(struct player *pplayer)
{
  ...
  cplayer->government = game.ruleset_control.default_government;
