...
/* NB Must match enum action_pattern_type
   in connecthand.h */
char *user_action_pattern_type_strs[NUM_ACTION_PATTERN_TYPES] = {
  "address",
  "hostname",
  "username"
};
                            ...
                            int pref)
{
  bool match = FALSE;
  user_action_list_iterate (on_connect_user_actions, pua) {
    freelog (LOG_DEBUG, "testing action [%s %s:%s] against %s "
             " (username=\"%s\", pref=%d)",
             user_action_strs[pua->action],
             user_action_pattern_type_strs[pua->type],
             pua->pattern, conn_description (pconn),
             username, pref);
    if (0 <= pref && pref < NUM_ACTIONS && pua->action != pref)
      continue;
            
    if (pua->type == APT_ADDRESS) {
      match = wildcardfit (pua->pattern, pconn->server.ipaddr);
    } else if (pua->type == APT_HOSTNAME) {
      match = wildcardfit (pua->pattern, pconn->addr);
    } else if (pua->type == APT_USERNAME) {
      match = wildcardfit (pua->pattern, username ? username
                           : pconn->username);
		}
    if (match) {
      freelog (LOG_DEBUG, "  matched! returning action %d",
               pua->action);
      return pua->action;
    }
  } user_action_list_iterate_end;
                          ...
                          struct packet_server_join_req *req)
{
  ...
  if((game.maxconnections!=0) && (conn_list_size(&game.all_connections) > game.maxconnections)) {
		reject_new_connection(_("Maximum number of connections for this server exceeded."),pconn);
		return FALSE;
	}
