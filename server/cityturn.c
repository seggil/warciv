...
static bool upgrade_building_prod(struct city *pcity)
{
  ...
  if (!pcity->server.delayed_build && ptype && ptype->obsolete_by != A_LAST) {
    /* Delay this */
    pcity->server.delayed_build = TRUE;
    return FALSE;
  }

  pplayer = city_owner(pcity);
  upgrades_to = building_upgrades_to(pcity, pcity->currently_building);
...
static bool upgrade_unit_prod(struct city *pcity)
{
  ...
  id2 = unit_upgrades_to(pcity, pcity->currently_building);
  ...
  if (can_build_unit_direct(pcity, id2)) {
    if (pcity->server.delayed_build) {
      ...
      if (pcity->shield_stock >= unit_build_shield_cost(id)
	  && pcity->shield_stock < unit_build_shield_cost(id2)) {
	return TRUE;
      }
    }
...
static bool city_build_building(struct player *pplayer, struct city *pcity)
{
  ...
  if (pcity->shield_stock
    ...
    if (is_wonder(pcity->currently_building)) {
      game.info.global_wonders[pcity->currently_building] = pcity->id;
