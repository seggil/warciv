...
static bool unban_command(struct connection *caller, char *pattern, bool check);
static bool addaction_command(struct connection *caller, char *pattern, bool check);
static bool delaction_command(struct connection *caller, char *pattern, bool check);
...
static void check_vote(struct voting *vote)
{
  ...
  if (vote->command[0] != '\0'
          ...
          || (num_cast > num_voters / 2)))
    {
        ...
        if ( ((vote->yes > vote->no)&&(command_named(vote->command,FALSE)!=CMD_END_GAME))||
        (command_named(vote->command,FALSE)==CMD_END_GAME  && (num_cast > num_voters / 2) && vote->yes > 0 && vote->no == 0) ) {
          /* Do it! */
          notify_conn(NULL, _("Vote \"%s\" is passed %d to %d with %d "
    			  "abstentions."), vote->command, vote->yes, vote->no,
    		  num_voters - vote->yes - vote->no);
          for (i = 0; i < MAX_NUM_PLAYERS; i++)
                switch(vote->votes_cast[i]) {
                    case VOTE_YES:
                        notify_conn(NULL,_("Vote \"%s\" %s voted yes"),vote->command,game.players[i].name);
                        break;
                    case VOTE_NO: 
                        notify_conn(NULL,_("Vote \"%s\" %s voted no"),vote->command,game.players[i].name);
                        break;
                    default:
                ;}
...
static bool remove_player(struct connection *caller, char *arg, bool check)
{
  ...
  if (!(game.is_new_game && (server_state==PRE_GAME_STATE ||
    ...
    cmd_reply(CMD_REMOVE, caller, C_OK, _("Removed player %s from the game."), name);
...
static bool observe_command(struct connection *caller, char *str, bool check)
{
  ...
  if (!pplayer) {
    const char *allow; 
    if (!(allow=strchr(game.allow_take, (game.is_new_game ? 'O' : 'o')))) {
	    cmd_reply(CMD_OBSERVE, caller, C_FAIL, _("Sorry, one can't observe globally in this game."));
	    goto end;
    }
    allow++;
    if (*allow == '2' || *allow == '3') {
...
if (load_successful) my_snprintf(buffer, sizeof(buffer),
    _("Mapload: map %s has been sucsessfully loaded"), srvarg.load_filename);
...
{
  if (server_state != PRE_GAME_STATE) {
    cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("Can't load a new map while a game "
                                          "is running."));
  ...
  bool isnumber=TRUE;
  for (cpstr_a=str; *cpstr_a != '\n' && !is_ok_mapfilename_char(*cpstr_a); cpstr_a++) {l++;}

  for (cpstr_b=_arg; *cpstr_a != '\n' && is_ok_mapfilename_char(*cpstr_a); cpstr_a++ ,cpstr_b++) {
    *cpstr_b=*cpstr_a;
    arglen++;
    l++;
    }
  *cpstr_b='\n';
  ...
  if (l<strlength) {
    ...
    cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("There are some unallowed chars or to many "
  ...
  if(isnumber) {
    ...
    if (!section_file_load_nodup(&maplistfile, maplistname)) {
      cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("There is no Maplistfile %s, you can't "
    ...
    if (!sscanf(_arg,"%d",&mapnum)) {
      cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("somehow system thought %s is a number, "
				"so it could not load the map"), _arg);
      return FALSE;
      }
    if (mapnum>=maxmapnum) {
      cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("There are only %d maps in the maplist, "
				"you tried to load map number %d"), maxmapnum,mapnum);
      return FALSE;
      }
    sz_strlcpy(mapname,secfile_lookup_str(&maplistfile,"list.m%03d",mapnum));
    }
  else {
  ...
  /* attempt to parse the file */
    if (!section_file_load_nodup(&file, arg)) {
      cmd_reply(CMD_LOADMAP, caller, C_FAIL, _("Couldn't load mapfile: %s"), arg);
      send_loadmap_game_info(FALSE);
      return FALSE;
      }
  ...
  /* Everything seemed to load ok; spread the good news. */
    ...
    cmd_reply(CMD_LOADMAP,caller,C_OK, _("Server: loaded mapfile: %s"), arg);
    return TRUE;
    }
  else {
    char arg[strlen(filename) + 1];
    sz_strlcpy(arg, filename);
    cmd_reply(CMD_LOADMAP,caller,C_FAIL, _("Server could not load the mapfile: %s, "
...
static bool showmaplist_command(struct connection *caller)
{
  ...
  if (!section_file_load_nodup(&file, filename)) {
    cmd_reply(CMD_SHOWMAPLIST, caller, C_FAIL, _("There is no Maplistfile %s"), filename);
    return FALSE;
    }
  int i;
  int mapnum = secfile_lookup_int(&file,"list.number");
  cmd_reply(CMD_SHOWMAPLIST, caller, C_COMMENT, _("You can load the following %d maps with the command /loadmap <number of map in list>"),mapnum);
  for (i=0;i<mapnum;i++) {
    char mapname[512];
    char comment[1024];
    sz_strlcpy(mapname,secfile_lookup_str(&file,"list.m%03d",i));
    cmd_reply(CMD_SHOWMAPLIST, caller, C_COMMENT, _("Map %d : %s"),i ,mapname);
    ...
    if(dfilename) {
      ...
      sz_strlcpy(comment,secfile_lookup_str_default(&mapfile,"No comment available","game.comment"));
      cmd_reply(CMD_SHOWMAPLIST, caller, C_COMMENT, _("Comment of map %d : %s"),i ,comment);
      }
    else {
      cmd_reply(CMD_SHOWMAPLIST, caller, C_FAIL, _("Map is not there"));
      }
  ...
  if ((p = strchr(tmp, '='))) {
    *p++ = 0;
    ...
    if (type == NUM_ACTION_PATTERN_TYPES) {
      my_snprintf (errbuf, errbuflen, _("Invalid pattern type \"%s\""),
                   tmp);
      return FALSE;
    }
  } else {
                                  ...
                                  bool check)
{
  ...
  *p++ = 0; /* Mark end of <action> string */
  ...
  if (!parse_action_pattern (p, buf, sizeof (buf), &type,
                             err, sizeof (err)))
  {
    cmd_reply (CMD_ADDACTION, caller, C_SYNTAX,
  ...
  cmd_reply (CMD_ADDACTION, caller, C_COMMENT,
             ...
             user_action_pattern_type_strs[pua->type],
             pua->pattern);
  return TRUE;
}
...
...
**************************************************************************/
static bool ban_command (struct connection *caller,
                         char *pattern, 
                                  bool check)
{
	struct user_action *pua;
                           ...
                           bool check)
{
  ...
  user_action_list_iterate (on_connect_user_actions, pua) {
    if (pua->action == ACTION_BAN &&
        pua->type == type &&
        0 == mystrcasecmp (buf, pua->pattern))
    {
      found = TRUE;
      user_action_list_unlink (&on_connect_user_actions, pua);
      free (pua->pattern);
  ...
  if (found) {
    cmd_reply (CMD_UNBAN, caller, C_COMMENT,
               ...
               user_action_pattern_type_strs[type],
               buf);
  } else {
    cmd_reply (CMD_UNBAN, caller, C_FAIL,
                             ...
                             char *filename, bool check)
{
  ...
  if (ret == -1) {
    cmd_reply (CMD_LOADACTIONLIST, caller, C_FAIL,
               _("Could not load action list from %s."),
               filename);
    return FALSE;
  } 
...
static int load_action_list_v0 (const char *filename)
{
  ...
  while (fgets (line, sizeof(line), file)) {
    ...
    if(sscanf(p,"%s %d",addr,&actionid)==2) {
			...
			if(actionid < 0 || actionid >= NUM_ACTIONS) continue;
    ...
    *p++ = 0;
    ...
    if (q) {
      *q++ = 0;
      type = NUM_ACTION_PATTERN_TYPES;
      for (i = 0; i < NUM_ACTION_PATTERN_TYPES; i++) {
        if (!mystrcasecmp (p, user_action_pattern_type_strs[i])) {
          type = i;
          break;
        }
      }
      if (type == NUM_ACTION_PATTERN_TYPES) {
        freelog (LOG_ERROR, _("Syntax error on line %d of"
            " action list file %s: unrecognized pattern type"
            " \"%s\"."), lc, filename, p);
        continue;
      }
    } else {
